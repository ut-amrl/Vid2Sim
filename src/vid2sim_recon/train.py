...
from utils.loss_utils import l1_loss, patch_based_ncc_loss, angular_loss, geo_consist_loss
from gaussian_renderer import render, network_gui
import sys
from scene import Scene, GaussianModel
from utils.general_utils import safe_state, get_expon_lr_func, render_normal
import uuid
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    depth_loss_weight = get_expon_lr_func(opt.depth_l1_weight_init, opt.depth_l1_weight_final, max_steps=opt.iterations)
    normal_loss_weight = get_expon_lr_func(opt.normal_l1_weight_init, opt.normal_l1_weight_final, max_steps=opt.iterations)
    geo_loss_weight = get_expon_lr_func(opt.geo_consist_weight_init, opt.geo_consist_weight_final, max_steps=opt.iterations)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        depth, normals, alpha = render_pkg["depth"], render_pkg["normals"], render_pkg["alpha"]

        # Loss
        gt_image = viewpoint_cam.original_image.cuda()
        gt_mask = viewpoint_cam.gt_alpha_mask.cuda() if viewpoint_cam.gt_alpha_mask is not None else torch.ones((gt_image.shape[1], gt_image.shape[2]), device="cuda")
        
        image = image * gt_mask
        gt_image = gt_image * gt_mask

        Ll1 = l1_loss(image, gt_image)
        ssim_val = fused_ssim(image.unsqueeze(0).float(), gt_image.unsqueeze(0).float())
        ...
        if iteration > opt.geo_sup_from_iter and iteration < opt.geo_sup_until_iter and viewpoint_cam.gt_depth is not None:
            # if viewpoint_cam.conf_map is not None:
            #     conf_map = viewpoint_cam.conf_map.to("cuda")
            #     gt_mask = gt_mask * conf_map

            gt_inv_depth = (viewpoint_cam.gt_depth.to("cuda")).float()
            gt_depth = 1.0 / (gt_inv_depth + 1e-9)
            gt_depth = torch.clip(gt_depth, 0.0, 150.0)
            depth = torch.clip(depth, 0.0, 150.0)
            ...
            depth_loss = patch_based_ncc_loss(inv_depth, gt_inv_depth, gt_mask)
            normal_loss = angular_loss(normals, gt_normal, gt_mask*inf_depth_mask)
            geo_consistency_loss = geo_consist_loss(normals, gt_depth, gt_mask*inf_depth_mask)

            loss += depth_loss_weight(iteration) * depth_loss
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f} | GS Num: {scene.gaussians.get_xyz.shape[0]}"})
